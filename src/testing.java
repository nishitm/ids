...
public class testing {
	...
	//		 System.out.println("#no of commands(value of m)-"+temps.size());
		 ...
		 for(int i=0; i<temps.size();i++)
			 Final[i] = temps.get(i);
		 System.out.println("Final-"+Final.length);
		 int c= Final.length/100;
	...
	/*	 for(int i=0; i<user_segment.size();i++)
		 ...
		 System.out.println("segment-"+user_segment.size());
		 for(int i=0; i<user_segment.size(); i++)
		 {
			// System.out.println("--------"+i);
			 ...
			 for(int j=0; j<t_data; j++)
			 {
			//	 System.out.println(j+"->"+user_segment.get(i)[j]);
				 if(!temps.contains(user_segment.get(i)[j]) && user_segment.get(i)[j]  != Character.toString('['))
				 	temps.add(user_segment.get(i)[j]);
			 }
		 }
		 int d = Final.length%100;
		 for(int j=0; j<d; j++)
		 {
			 if(!temps.contains(user_segment.get(c)[j]) && user_segment.get(c)[j]  != Character.toString('['))
				 	temps.add(user_segment.get(c)[j]);
		 }
	//	 temps.remove("[");
		 System.out.println("#no of commands(value of m)-"+(temps.size()));
		 FileWriter file = new FileWriter("/home/nishit/workspace/CS631_Project_new/B/testing.csv");
		...
		//	 System.out.printf("\n");
		 ...
		 for(int i=0; i<user_segment.size(); i++)
		 {
			 FileWriter fileWriter = new FileWriter("/home/nishit/workspace/CS631_Project_new/B/"+i+".csv");
			 for(int j=0; j<temps.size();j++)
			 {
